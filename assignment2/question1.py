...
class Content(object):
    ...
    def show(self):
        print "<Title: {0}\nSubtitle: {1}\nCreator: {2}\nPublication Date: {3}>".format(self.title, self.subtitle, self.creator, self.publication_date.__str__())

    def matches_url(self, url):
        pattern = r'http://thecrimson.com/(\w+)/(\d{4})/(\d{1,2})/(\d{1,2})/(\w+)'
        return not (re.search(pattern, url) is None)
    ...
    def from_url(c_lst, url):
        ...
        for i in range(len(c_lst)):
            ...
            pattern = r'http://thecrimson.com/(\w+)/{0}/{1}/{2}/{3}/'.format(c_list[i].publication_date.year, c_list[i].publication_date.month, c_list[i].publication_date.day, slug)
            if (not (re.search(pattern, url) is None)):
                matched_content.append(c_lst[i])
...
class Article(Content):
    ...
    def show(self):
        print "<Title: {0}\n Subtitle: {1}\nCreator: {2}\nPublication Date: {3}\nRelated Image: {4}>".format(self.title, self.subtitle, self.creator, self.publication_date.__str__(), self.related_image)
...
class Picture(Content):
    ...
    def show(self):
        print "<Title: {0}\n Subtitle: {1}\nCreator: {2}\nPublication Date: {3}\nImage File: {4}>".format(self.title, self.subtitle, self.creator, self.publication_date.__str__(), self.image_file)
...
def posted_after(c_lst, dt):
    ...
    for content in c_lst:
        if (content.publication_date < dt): 
            content_before_dt.append(content)
