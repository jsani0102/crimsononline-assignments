...
def save_author(request):
    ...
    if request.method == 'POST':
        success = True
        try:
            # Find the first and last names of the created author as well as the gender
            f = request.POST['first'].strip()
            l = request.POST['last'].strip()
            g = request.POST['gender'].strip()
            # Create a new Author object with form's data
            new_author = Author(first=f, last=l, gender=g)
            # Validate and save the BlogPost
